...
/**
 * This class generates a sample document in case no document has been passed.
 * ConvertOutPDF.java에서 사용하는 클래스(입력docx가 없을떄 더미 docx를 생성)
 */
public class SampleDocument {

	public static void createContent(MainDocumentPart wordDocumentPart) {
		/*
		 * NB, this currently works nicely with viaIText, and viaXSLFO (provided
		 * you view with Acrobat Reader .. it seems to overwhelm pdfviewer,
		 * which is weird, since viaIText works in both).
		 */
	...
	final static String	sampleTextBoldItalic	= "<w:p xmlns:w=\"http://schemas.openxmlformats.org/wordprocessingml/2006/main\">"
														...
														+ "<w:i />"
														+ "</w:rPr>"
														+ "<w:t>${fontname} bold italic</w:t>" + "</w:r>" + "</w:p>";
