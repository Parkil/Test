...
public class DDEXOcdXmlParsing {
	static Logger logger = LoggerFactory.getLogger(DDEXOcdXmlParsing.class);
	...
	public Map<String,Object> getAbmInfo(Document doc, XPath xpath, String territoryCode) throws XPathExpressionException, ParseException {
		...
		Node AbmTakeDownNode = (Node)xpath.evaluate("//ReleaseDeal/DealReleaseReference[text() = 'R0']/..//TakeDown", doc, XPathConstants.NODE);
		if(AbmTakeDownNode != null) {
			abmInfoMap.put("ABM_DELETE", AbmTakeDownNode.getTextContent());
		}
		
		Node AbmUpdateIndiNode = (Node)xpath.evaluate("//UpdateIndicator", doc, XPathConstants.NODE);
		if(AbmUpdateIndiNode != null) {
			abmInfoMap.put("ABM_TAKEDOWN", AbmUpdateIndiNode.getTextContent());
		}
		
		NodeList abmEndDateNode = (NodeList)xpath.evaluate("//ReleaseDeal/DealReleaseReference[text() = 'R0']/../EffectiveDate", doc, XPathConstants.NODESET);
	...
	public Map<String,List<Map<String,Object>>> getSngInfo(Document doc, XPath xpath, String territoryCode) throws XPathExpressionException, ParseException{
		...
		while(listSize != 0) {
			...
			for(int i=0 ; i<listSize ; i++) {
				...
				Node assetRefIdNode = (Node)xpath.evaluate("//Release[not(@IsMainRelease)]/ReleaseResourceReferenceList/ReleaseResourceReference[text() = '"+assetId+"']/../../ReleaseReference", doc, XPathConstants.NODE);
				String assetRefId = assetRefIdNode.getTextContent(); //곡 발매정보를 가지고 있는 ID R1,R2....
				
				map.put("assetRefId", assetRefId);
				...
				Node formalRepNmNode = (Node)xpath.evaluate("//SoundRecording/ResourceReference[text() = '"+assetId+"']/..//Title[@TitleType='OriginalTitle']/TitleText", doc, XPathConstants.NODE);
				...
				Node sngTakeDownNode = (Node)xpath.evaluate("//ReleaseDeal/DealReleaseReference[text() = '"+assetRefId+"']/..//TakeDown", doc, XPathConstants.NODE);
				if(sngTakeDownNode != null) {
					map.put("SNG_DELETE", sngTakeDownNode.getTextContent());
				}
				
				Node SngUpdateIndiNode = (Node)xpath.evaluate("//UpdateIndicator", doc, XPathConstants.NODE);
				if(SngUpdateIndiNode != null) {
					map.put("SNG_TAKEDOWN", SngUpdateIndiNode.getTextContent());
				}
				
				NodeList sngEndDateNode = (NodeList)xpath.evaluate("//ReleaseDeal/DealReleaseReference[text() = '"+assetRefId+"']/../EffectiveDate", doc, XPathConstants.NODESET);
	...
	public static void main(String[] args) throws Exception{
		Document doc = new DDEXOcdXmlParsing().getDocumnet("d:\\xml_ord\\192562380369.xml");
		...
		/*
		Node test = (Node)xpath.evaluate("//Release[not(@IsMainRelease)]/ReleaseResourceReferenceList/ReleaseResourceReference[text() = 'A1']/../../ReleaseReference", doc, XPathConstants.NODE);
		System.out.println(test.getTextContent());
		*/
		/**/
		DDEXOcdXmlParsing parse = new DDEXOcdXmlParsing();
