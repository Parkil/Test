...
/*
 * Class Loader를 Overriding 하여 Class loading중에 클래스파일을 수정한다.
 */
public class LogMethodsClassLoaderBCEL extends ClassLoader {
	...
	protected JavaClass modifyClass(JavaClass clazz) {
		...
		LogMethodsClassLoaderBCEL.out = cp.addFieldref("java.lang.System",
				"out", "Ljava/io/PrintStream;");
		LogMethodsClassLoaderBCEL.println = cp.addMethodref(
		...
		for (int i = 0; i < methods.length; i++) {
			...
			mesg += Utility.methodSignatureToString(methods[i].getSignature(),
					name, Utility.accessToString(flags));
